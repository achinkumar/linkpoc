...
var OauthStep2 = function (request, response, code) {
	...
	var options = {
		...
		port: 443,
		path: "https://www.linkedin.com/uas/oauth2/accessToken?grant_type=authorization_code&code=" + code + "&redirect_uri=" + callbackURL + "&client_id=" + APIKey + "&client_secret=" + APIKeySecret
	};
	...
	var req = https.request(options, function (res) {
		...
		res.on('data', function (d) {
			...
			ExpiresIn29days.setDate(ExpiresIn29days + 29);
			response.writeHead(201, {
				'Set-Cookie': 'LIAccess_token=' + access_token + '; Expires=' + ExpiresIn29days
			});

			OauthStep3(request, response, access_token, APICalls['myNetworkShares']);
		});
	});
...
APICalls['myStatus'] = 'people/~:(current-status)';
...
APICalls['peopleSearchWithKeywords'] = 'people-search:(people:(id,first-name,last-name,picture-url,headline),num-results,facets)?keywords=Saurabh+Sharma';
